...
time_array = ["µs", "ms", "ms", "ms", "s"]

files = "/home/neville/git/cucosma/Benchmarks/SwizzleBenchmarks/swizzle.csv_nid01982_2020-09-13_16:56:11/nvprof_*.csv"
# print(files)
...
for i, filename in enumerate(natsort.natsorted(glob.glob(files), reverse=False)):
    ...
    if i % 16 == 15:
        ...
        # plt.show()
        plt.savefig("../thesis/images/swizzle_plot_P100_" + names[j] + ".pdf", format="pdf", bbox_inches="tight")
        plt.clf()
        data = pd.DataFrame(dtype='float')
        j -= 1
        # plt.savefig("../thesis/images/swizzle_plot_P100.pdf", format="pdf", bbox_inches="tight")
