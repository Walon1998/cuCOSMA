...
def plot(tuple):
    ...
    name_file = "../thesis/images/flops_square_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    #name_file = "./flops_square_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    ax.set(ylabel="TFLOPS", xlabel=r'$\omega$', title=r'M = $\omega$, N = $\omega$, K = $\omega$')
...
data = pd.read_csv("data_square_flops.csv")
# data_cosma = pd.read_csv("data_square_cosma.csv")

# data = data.append(data_cosma)
print(data)

tuples = [(0, 512), (512, 1024), (1024, 4096), (4096, 10000), (10000, 16384), (0, 16384)]
