...
def plot(tuple):
    ...
    name_file = "../thesis/images/Speedup_flat_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    ax.set(ylabel="Speedup", xlabel=r'$\omega$',title=r'M = $\omega^2$, N = $\omega^2$, K = $\omega$')
...
data = pd.read_csv("data_Flat.csv")
