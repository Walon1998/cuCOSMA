...
configs = "/home/neville/git/cucosma/Benchmarks/128x128x8_Benchmarks/square*/config*"
nvprofs = "/home/neville/git/cucosma/Benchmarks/cosmaBenchmarks/square*/nvprof*"
configs = "/home/neville/git/cucosma/Benchmarks/cosmaBenchmarks/square*/config*"
...
for i in range(len(configs_array)):
    ...
    #print((M * N * (2 * K + 3)) / (col_cublas_kernel * 10e3))

    curr_df_cublas['Omega'] = numpy.full(100, M)
    curr_df_cublas['Implementation'] = numpy.full(100, cublas_real_name)
    curr_df_cublas['Time'] = (M * N * (2 * K + 3)) / (col_cublas_kernel * 10e5)

    curr_df_cutlass['Omega'] = numpy.full(100, M)
    curr_df_cutlass['Implementation'] = numpy.full(100, "CUTLASS")
    curr_df_cutlass['Time'] = (M * N * (2 * K + 3)) / (col_cutlass * 10e5)

    curr_df_cosma['Omega'] = numpy.full(100, M)
    curr_df_cosma['Implementation'] = numpy.full(100, "cuCOSMA")
    curr_df_cosma['Time'] = (M * N * (2 * K + 3)) / (col_cosma * 10e5)
...
data.to_csv("data_square_flops.csv", index=False)
