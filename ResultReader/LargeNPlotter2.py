...
def plot(tuple):
    ...
    name_file = "../thesis/images/simple_flops_LargeN_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    #name_file = "Speedup_LargeN_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    ax.set(ylabel="TFLOPS", xlabel=r'$\omega$', title=r'M = $\omega$, N = $\omega^2$, K = $\omega$')
...
data = pd.read_csv("data_LargeN_flops2.csv")
