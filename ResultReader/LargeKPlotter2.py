...
def plot(tuple):
    ...
    name_file = "../thesis/images/flops_LargeK_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    ax.set(ylabel="TFLOPS", xlabel=r'$\omega$', title=r'M = $\omega$, N = $\omega$, K = $\omega^2$')
...
data = pd.read_csv("data_LargeK_flops.csv")
