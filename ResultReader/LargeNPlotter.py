...
def plot(tuple):
    ...
    name_file = "../thesis/images/simple_Speedup_LargeN_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    #name_file = "Speedup_LargeN_" + str(tuple[0]) + "-" + str(tuple[1]) + ".pdf"
    ax.set(ylabel="Speedup", xlabel=r'$\omega$', title=r'M = $\omega$, N = $\omega^2$, K = $\omega$')
...
data = pd.read_csv("data_LargeN2.csv")
