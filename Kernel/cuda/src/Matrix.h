...
//			std::cout << "Implementation seems correct!" << std::endl;
	...
	Matrix CUDAMatrixMultiply2DBoundChecking(const Matrix<T> &B) {
		...
		checkCudaErrors(cudaMemcpy(Cuda_A.elems, elems, size_A, cudaMemcpyHostToDevice));
		checkCudaErrors(cudaMemcpy(Cuda_B.elems, B.elems, size_B, cudaMemcpyHostToDevice));
		...
		checkCudaErrors(cudaMemcpy(C.elems, Cuda_C.elems, size_C, cudaMemcpyDeviceToHost));
	...
	Matrix CUDAMatrixMultiply2DNonBoundChecking(const Matrix<T> &B) {
		...
		for (int i = 0; i < C.rows; i++) {
			...
			gpuErrchk(cudaMemcpy(&C.elems[i * C.cols], &Cuda_C.elems[i * Cuda_C.cols], C.cols * sizeof(T), cudaMemcpyDeviceToHost));
	...
	Matrix CUDAMatrixMultiply2DNonBoundCheckingShared(const Matrix<T> &B) {
		...
		for (int i = 0; i < C.rows; i++) {
			checkCudaErrors(cudaMemcpy(&C.elems[i * C.cols], &Cuda_C.elems[i * Cuda_C.cols], C.cols * sizeof(T), cudaMemcpyDeviceToHost));
