...
#define THREADS ((THREADBLOCK_TILE_M / WARP_TILE_M) * (THREADBLOCK_TILE_N / WARP_TILE_N) * 64)
...
__device__ __inline__ void load_C_Single(TYPE * __restrict__ Thread_Tile,
		...
		TYPE (* __restrict__ Shared)[(THREADBLOCK_TILE_M / WARP_TILE_M)
				* (THREADBLOCK_TILE_N / WARP_TILE_N) * 192]) {
				...
				if(threadIdx.x == 0){
				printf("Epilogue not supported in hipCOSMA");
				}
				return;
...
cosmaSgemm_kernel(const TYPE * __restrict__ A, const int lda,
	...
	static_assert(N_THREADS * M_THREADS == 64, "Warp has 64 Threads");

	const int WarpId = threadIdx.x / 64;
	const int threadId = threadIdx.x % 64;

	const int WarpIdx = WarpId % N_WARPS;
	const int WarpIdy = WarpId / N_WARPS;

	int LaneIdx = threadId % N_THREADS;
	int LaneIdy = threadId / N_THREADS;
 ...
 * @param C			<type> array of dimensions ldc x n
 * @param ldc		leading dimension of a two-dimensional array used to store the matrix C.
 * @return hipblasStatus_t HIPBLAS_STATUS_SUCCESS || HIPBLAS_STATUS_NOT_INITIALIZED || HIPBLAS_STATUS_INVALID_VALUE || HIPBLAS_STATUS_ARCH_MISMATCH || HIPBLAS_STATUS_EXECUTION_FAILED
 */
bool cosmaSgemm(const TYPE *__restrict__ A, const int lda,
		const TYPE * __restrict__ B, const int ldb, TYPE *__restrict__ C,
		const int ldc) {
	...
	if (BETA != 1 && ((SPLIT_K > 1 && ATOMIC_REDUCTION) || ALPHA == 0)) {
		...
		//hipblasSscal(handle, ldc * M, &factor, C, 1);
	...
	if (ALPHA != 0) {
		...
		if (SPLIT_K != 1 && !ATOMIC_REDUCTION) {
			...
			TYPE* C_SPLITK;
					...
					hipMalloc(&C_SPLITK, sizeof(TYPE) * M * N * SPLIT_K);

			hipLaunchKernelGGL(cosmaSgemm_kernel, dim3(dimGrid), dim3(dimBlock), 0, 0, A,lda,B,ldb,C_SPLITK,N);
			...
			hipLaunchKernelGGL(cosmaSgemm_kernel, dim3(dimGrid), dim3(dimBlock), 0, 0, A,lda,B,ldb,C,ldc);
		}
