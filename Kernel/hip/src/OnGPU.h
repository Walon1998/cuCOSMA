...
void OnGPUNaive(const T &A, const T &B, const T &C, const float alpha,
	...
	hipLaunchKernelGGL(OnGPUNaive_Kernel, dim3(dimGrid), dim3(dimBlock), 0, 0, A, B, C, alpha, beta, lda, ldb,
			ldc);
	hipGetLastError();
...
void OnGPUShared(const T &A, const T &B, const T &C, const float alpha,
	...
	hipLaunchKernelGGL(OnGPUShared_Kernel, dim3(dimGrid), dim3(dimBlock), 0, 0, A, B, C, alpha, beta, lda, ldb,
			ldc);
	hipGetLastError();
	hipDeviceSynchronize();
